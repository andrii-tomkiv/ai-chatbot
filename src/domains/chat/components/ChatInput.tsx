...
export default function ChatInput({
  ...
  return (
    <div className="p-3 sm:p-6 border-t border-white/20 bg-gradient-to-r from-white/90 to-conab-light-background/90 backdrop-blur-sm">
      {rateLimitStatus && rateLimitStatus.remaining <= 2 && rateLimitStatus.remaining > 0 && rateLimitStatus.allowed && (
        <div className="mb-3 sm:mb-4 p-2 sm:p-3 bg-yellow-100/80 border border-yellow-300/50 rounded-xl text-yellow-800 text-xs sm:text-sm backdrop-blur-sm">
      ...
      {rateLimitStatus && !rateLimitStatus.allowed && !rateLimitStatus.isBlocked && (
        <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-red-100/90 border-2 border-red-400/60 rounded-xl text-red-900 text-sm sm:text-base font-medium backdrop-blur-sm shadow-lg">
      ...
      {rateLimitStatus && rateLimitStatus.isBlocked && (
        <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-red-100/90 border-2 border-red-500/60 rounded-xl text-red-900 text-sm sm:text-base font-medium backdrop-blur-sm shadow-lg">
          🚫 <strong>You are blocked!</strong> Blocked for {getBlockedTime()} due to invalid messages. Please wait before trying again.
        </div>
      )}
      
      {rateLimitLoading && (
        <div className="mb-3 sm:mb-4 p-2 sm:p-3 bg-blue-100/80 border border-blue-300/50 rounded-xl text-blue-800 text-xs sm:text-sm backdrop-blur-sm">
          ⏳ Checking rate limit status...
        </div>
      )}
      
      <div className={`flex items-end space-x-2 sm:space-x-3 backdrop-blur-sm rounded-2xl p-3 sm:p-4 border shadow-lg ${
        isDisabled 
        ...
        <textarea
          ...
          className={`flex-1 px-0 py-2 border-none bg-transparent focus:outline-none resize-none text-sm sm:text-base leading-relaxed ${
            isDisabled 
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          ...
          <button
            ...
            className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl focus:outline-none transition-all duration-200 flex items-center justify-center shadow-lg ${
